...
public class AnimProcessor implements IAnimRefresh, IAnimOverScroll {
		...
		animLayoutByTime(getVisibleFootHeight(), 0, new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				if (!ScrollingUtil.isViewToBottom(cp.getTargetView(), cp.getTouchSlop())) {
					int dy = getVisibleFootHeight() -(Integer)animation.getAnimatedValue();
	...
	/**
	 * 7.执行顶部越界 To executive cross-border springback at the top. 越界高度height ∝
	 * vy/computeTimes，此处采用的模型是 height=A*(vy + B)/computeTimes
	 *
	 * @param vy
	 *            满足越界条件的手指滑动速度 the finger sliding speed on the screen.
	 * @param computeTimes
	 *            从满足条件到滚动到顶部总共计算的次数 Calculation times from sliding to top.
	 */
